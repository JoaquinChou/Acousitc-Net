...
from networks.CNN_for_sound_pressure import Sound_Pressure_CNN
...
wandb.init(
    ...
    entity='joaquin_chou',
    name="CNN_for_sound_pressure_l1_only_pressure" + "-epoch" + str(
        args.epochs) + "-lr" + str(
        args.lr),
    config=args
)
...
# 保存距离
# coding=UTF-8
filename = '/home2/zgx/data/single_sound_source_only_pressure_models/distance_val_2CONV_CNN_only_pressure.txt'
# 定义训练过程
...
for epoch in range(args.start_epoch, args.epochs + 1):
    ...
    model.train()
    # location,
    for batch_idx, (raw_sound_data, pressure) in enumerate(train_dataloader):
        ...
        output_pressure = model(raw_sound_data)
    ...
    if (epoch % 1 == 0):
        ...
        with torch.no_grad():
            total_val_loss = 0.
            # val_location,
            for batch_idx, (val_raw_sound_data, val_pressure) in enumerate(val_dataloader):
                ...
                val_output_pressure = model(val_raw_sound_data)
    ...
    if (epoch % 10 == 0) or (epoch == args.epochs + 1):
        torch.save(model.state_dict(),
                   args.model_dir + 'single_sound_source_CNN_CONV2_only_pressure' + str(epoch) + '.pth')
