...
parser = argparse.ArgumentParser(description='Propert Repvgg_A1 for single_sound_source in pytorch')
parser.add_argument('--arch', dest='arch', metavar='ARCH', default='RepVGG-A1')
...
wandb.init(
    ...
    entity='joaquin_chou',
    name="Sound_Pressure_RepVGG_A1__with_smoothl1_modified_pressure_level" + "-epoch" + str(
        args.epochs) + "-lr" + str(
        args.lr),
    config=args
)
...
lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                    milestones=[5, 50, 80], gamma=0.1, last_epoch=- 1)
# lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=4, verbose=True,
#                                                           threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0,
#                                                           eps=1e-08)
# 保存距离
# coding=UTF-8
filename = '/home2/zgx/data/single_sound_source_only_pressure_models/distance_val_Repvgg_A1_modified_pressure_level.txt'
# 定义训练过程
...
for epoch in range(args.start_epoch, args.epochs + 1):
    ...
    # 启动BN和dropout
    model.train()
    # location,, raw_sound_data
    for batch_idx, (stft_image, pressure) in enumerate(train_dataloader):
        ...
        output_pressure = model(input_var)
        ...
        l1 = criterion(output_pressure, pressure_var)
        batch_loss = l1
        # + kl
        ...
        if batch_idx % 5 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tl1 loss: {:.6f}\tAvg loss: {:.6f}'.
                format(
                epoch,
                (batch_idx + 1),
    ...
    lr_scheduler.step()
    print("------------------------------------------------------------------")
    print("Epoch: {}\tTime: {:.4f}\tTotal_Loss: {:.8f}\tLearningRate {:.4f}".format(epoch, time.time() - epoch_start_time,
    ...
    if (epoch % 3 == 0):
        ...
        with torch.no_grad():
            total_val_loss = 0.
            # val_location,, val_raw_sound_data
            for batch_idx, (stft_image, val_pressure) in enumerate(val_dataloader):
                ...
                val_output_pressure = model(val_input_var)
                ...
                # kl_loss = F.kl_div(val_output_constraint.softmax(dim=-1).log(), val_output_sound.softmax(dim=-1),
                #                    reduction='sum')
                l1_loss = criterion(val_output_pressure, val_pressure_var)
                pressure_error.append(l1_loss.cpu().numpy())

                total_val_loss += val_batch_loss.item()
                print('Val Epoch: {} [{}/{} ({:.0f}%)]\tl1 loss: {:.6f}\tAvg loss: {:.6f}'.
                    format(
                    epoch,
                    (batch_idx + 1),
                    len(val_dataloader),
    ...
    if (epoch % 1000 == 0) or (epoch == args.epochs + 1):
        torch.save(model.state_dict(),
                   args.model_dir + 'Repvgg_A1_modified_pressure_' + str(epoch) + '.pth')
